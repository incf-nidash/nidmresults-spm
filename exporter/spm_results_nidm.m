...
for c=1:numel(xSPM.Ic)
    ...
    if xSPM.STAT == 'T'
        ...
        nidm_contrasts(nidm_esc(SPM.xCon(xSPM.Ic(c)).name)) = ...
            containers.Map({...
...
for i = 2:3
    ...
    nidm_extent_equivthresh(nidm_esc(thresh(i).label)) = containers.Map(...
        {...
            ...
            'nidm_ExtentThreshold/prov:value'
        }, {...
            ex_equiv_types{i}, ...
            kk(i)
        });
...
nidm_inferences('nidm_ClusterDefinitionCriteria/nidm_hasConnectivityCriterion') = ...
    sprintf('nidm_voxel%dconnected',clusterConnectivityCriterion);
nidm_inferences('nidm_PeakDefinitionCriteria/nidm_minDistanceBetweenPeaks') = ...
    minDistanceBetweenPeaks;
nidm_inferences('nidm_PeakDefinitionCriteria/nidm_maxNumberOfPeaksPerCluster') = ...
...
nidm_inference('nidm_ExcursionSetMap/nidm_numberOfSupraThresholdClusters') = p;
...
nidm_inference('MIP') = uri(spm_file(files.mip,'cpath'));
...
for i=1:numel(idx)
    nidm_clusters(num2str(i)) = containers.Map(...
        {...
